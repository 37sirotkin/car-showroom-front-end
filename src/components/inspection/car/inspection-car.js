...
import "./inspection-car.scss";
import {AutoComplete} from "antd";
import {SearchOutlined} from "@ant-design/icons";
import {useDispatch, useSelector} from "react-redux";
import {getMarks} from "../../redux/actions/markActions";
import {getCars} from "../../redux/actions/carActions";
import InspectionCarItem from "./inspection-car-item";


const InspectionCar = ({setSelectCar, selectCar}) => {
                ...
                <div className="test-drive-car__items__row">
                    {
                        allViewCars.slice(0, 3).map((car, index) =>
                            <InspectionCarItem key={index} img={car.img}
                                               mark={selectMark}
                                               model={car.model}
                                               carId={car.id_car}
                ...
                <div className="test-drive-car__items__row">
                    {
                        allViewCars.slice(3, 6).map(car =>
                            <InspectionCarItem key={car.id_car} img={car.img}
